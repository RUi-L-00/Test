...
- (void)checkIsEmpty:(BOOL)isEmpty textField:(UITextField *)textField {
    if (JMIsEmpty(self.requiredTextFields)) {
        return;
    }
    
    if ([self respondsToSelector:@selector(textViewControllerRelationButtons:)]) {
        if (JMIsEmpty([self textViewControllerRelationButtons:self])) {
            return;
        }
    }else
    {
    ...
    if (!isEmpty) {
        [self.requiredTextFields enumerateObjectsUsingBlock:^(UITextField *obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj != textField && JMIsEmpty(obj.text.stringByTrim)) {
                isButtonEnabled = NO;
                *stop = YES;
            }
        }];
...
#pragma mark - JMTextViewControllerDataSource
...
#pragma mark - JMTextViewControllerTextField

static void *isEmptyAutoEnableKey = &isEmptyAutoEnableKey;
@implementation UITextField (JMTextViewController)
...
@implementation JMTextViewControllerTextField
- (NSString *)text {
